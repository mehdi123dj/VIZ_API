        ...
        def clicked_element_info(node, edge):
            ...
            if triggered == None:
                ...
                if changed_id == 'cytoscape.tapEdge':
                    ...
                    M = []
                    for elem in output:
                        #M.append(html.P(elem+" : " + str(output[elem]),style={'font-size': '1.6vmin'}))
                        if elem == 'data':
                            M.append(html.P('',style={'font-size': '1.6vmin'}))
                            for k,v in eval(output[elem]).items():
                                M.append(html.B(children= k ,style = {'color':'darkorchid','fontWeight':'bold'}))
                                M.append(html.P(v,style={'font-size': '1.6vmin'}))
